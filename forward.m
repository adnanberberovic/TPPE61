...
for day = size(midPrices,2):-1:1
    ...
    % While (seems to be enough with 2 iterations, always...)
    for i = 1:5
    ...
    title('Forward Rate Curves with LSExpSplines');
    forwardcurves(day,:) = fr;
end
forwardCurvesSplines = flipud(forwardcurves);
fileName = ['forwardCurveSplines' num2str(finalMaturity) '_' num2str(var) '_' num2str(p) '.mat'];
save(fileName,'forwardCurvesSplines');
